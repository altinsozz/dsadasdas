...
const bans = new Map();
  ...
  .setColor('#0070FF')
   ...
   .setFooter('Bu komutu kullanan kullanıcı ' + message.author.tag, message.author.avatarURL())
        ...
        const embed = new Discord.MessageEmbed()
            .addField(`Sunucuda Bulunan Yasaklılar`, '<:nooo:834402193351114772> Üzgünüm ama sunucunuzda fazla sayıda yasaklı kullanıcı bulunuyor Bu Yüzden gösteremiyorum. Discord buna izin vermiyor.')
            .setColor('RED')
          .setFooter('Bu komutu kullanan kullanıcı ' + message.author.tag, message.author.avatarURL())
            .setTimestamp()
        message.channel.send(embed)
