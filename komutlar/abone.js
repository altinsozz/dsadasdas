...
exports.run = async (client, message, args) => {
  ...
  if (!abonerol)
    return message.channel.send(
      `<:nooo:834402193351114772> **__Abone rolü ayarlanmamış!__**`
    );
  if (!abonelog)
    return message.channel.send(
      `<:nooo:834402193351114772> **__Abone log kanalı ayarlanmamış!__**`
    );
  if (!aboneyetkilisi)
    return message.channel.send(
      `<:nooo:834402193351114772> **__Abone yetkili rolü ayarlanmamış!__**`
    );
  ...
  if (!message.mentions.users.first())
    return message.channel.send(`<:nooo:834402193351114772> Bir Üye Etiketle!`);

  await abonekisi.roles.add(abonerol);
  const embed = new Discord.MessageEmbed()
    .setTitle(`<:Partner1:834402241208909835> Abone Rolü Verildi!`)
    .addField(
